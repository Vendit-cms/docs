...
1. **영어 그리드가 50,000 을 `+5만` 으로 찍는다.** 이건 번역 누락이 아니라 **통화 단위를 따라가는
   축약**이다(KRW 는 만, USD 는 K). 그래서 영어 UI 라도 원화 업장이면 한글 글자가 셀에 박힌다.
   모달 안에서는 `₩ +50,000` 으로 풀어 쓴다. 10,000 이상 값이면 조정행, 요금행 어디서나 재현된다.
   설계대로 동작하는 것이라도 **영어 문서로 한글이 새는 경로**라 촬영 중에는
   `install_guard()` 가 표기만 영어 자릿수로 되돌린다(값은 안 건드린다).
   제품에서 고칠 거라면 영어 로케일의 축약 단위를 통화가 아니라 표시 언어에 맞추는 쪽이다.
