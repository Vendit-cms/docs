...
NEEDS_DEAN = {
    "Distribution-Booking-Sync-Only.png": (
        "앱 버그다. 번역 문제가 아니다(2026-09-17 확인). 같은 키 "
        "app.sync-banner.description.stopped-inventory-and-rate 를 ko 도 en 도 {date} 자리표시자로 "
        "똑같이 들고 있는데, 한국어 화면은 '2026-07-27 18:32:25 이후,' 로 값이 박히고 영어 화면은 "
        "'Since {date},' 를 글자 그대로 찍는다. 로케일만 다르고 문자열·키·화면이 같으니 값을 꽂는 "
        "쪽이 영어에서만 안 도는 것이다. {date} 를 쓰는 키는 전체 30개라 이 배너만의 일도 아니다. "
        "vcms-web 이 고쳐야 풀린다. en-distribution-booking-sync-only.png 는 찍어뒀지만 그 캡처에도 "
        "{date} 가 그대로 찍혀서 못 쓴다"
    ),
}
