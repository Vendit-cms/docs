...
- **녹화 시작과 끝은 툴바의 확장 아이콘을 진짜 마우스로 눌러야 한다.** 확장 단축키 `⌘⇧8` 은 브라우저가
  먹는 가속키라 CDP 로 보낸 키는 닿지 않는다. 확장은 툴바 클릭(사용자 제스처)으로만 activeTab 권한을 받는다.
  pyobjc `Quartz.CGEventPost` 로 아이콘을 누르고, 뜬 `popup.html` 타깃 안의 버튼은 CDP 로 누른다.
  팝업 버튼은 `^Record Interactive Demo$` 로 정확히 찾아라. `Record` 로 느슨하게 찾으면 `Record Video` 가
  먼저 걸려 영상이 녹화된다(2026-09-19). 그래서 녹화는 헤드리스로 못 한다. 화면이 30초쯤 필요하다.
  페이지 안 클릭과 드래그는 CDP 로 보내도 녹화된다.
...
- 워크스페이스 기본 **인트로 챕터**가 1번 스텝으로 따라붙는다.
- **녹화가 끝난 뒤 편집은 `scripts/supademo_edit.py` 로 한다.** 에디터 화면이 필요 없다.
  `tidy` 가 인트로 챕터와 영상 스텝을 지우고, `texts` 가 핫스팟 문구를, `meta` 가 제목과 메타 설명을 바꾼다.
  (아웃트로 Tally 설문은 2026-09-12 에 Dean 이 워크스페이스 기본값에서 껐다.)
- AI Enhancement 가 핫스팟 문구, 제목, 메타 설명을 자동으로 붙인다. 틀린 게 많다(요금 데모에 체크인 설명이
  붙어 있었다). 전부 손으로 지정해라. 기존 번역 문구는 `scripts/demo_glossary_check.py` 의 `texts()` 로 긁을 수 있다.

## 운영 서버 경계

`www.vcms.io` VENDIT HOTEL 만 허용. 테스트 업장이고 연결된 채널이 0개라 밖으로 나가는 게 없다.
**상단 바 `Save` 를 눌러라**(Dean, 2026-09-18). 처음엔 누르지 말라고 적어뒀는데, 그 바람에 영어 데모에
적용된 화면과 컨페티가 통째로 빠졌다. Save 뒤 중립 클릭 한 번이 컨페티와 적용 결과를 마지막 스텝에 담는다.
테이크가 끝나면 저장한 값을 되돌려 다시 저장하고, 새로고침해서 원래대로인지 확인해라.
판매 재개는 상단 저장 전에 확인 팝오버가 따로 뜨는 경우가 있다. 그 안의 저장부터 눌러야 상단 바가 나온다.
