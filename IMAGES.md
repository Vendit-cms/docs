# 영어 문서 이미지 현황

`scripts/images_manifest.py` 가 en/ 의 실제 참조를 읽어 만든다. 손으로 고치지 마라.

`en-` 로 시작하는 파일은 영어 UI 에서 새로 찍은 것이다. 나머지는 한국어 원본을 그대로 쓴다.
`images/` 는 한국어 문서와 공유하므로 원본을 덮으면 한국어까지 바뀐다. 그래서 새 파일로 넣는다.

참조 274건 중 223건 정리 완료, 51건 남음.

## `en/accommodation/create-accommodation.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-accommodations-list.png` | 영어 캡처 완료 |
| `Inviting-1.png` | 재현 불가 — 대기 중인 초대가 실재해야 한다 |

## `en/accommodation/invite-user.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-users-list.png` | 영어 캡처 완료 |
| `en-users-invite.png` | 영어 캡처 완료 |
| `en-users-row-menu.png` | 영어 캡처 완료 |
| `en-users-edit-role.png` | 영어 캡처 완료 |

## `en/accommodation/signup-login.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-signin-email.png` | 영어 캡처 완료 |
| `en-signin-phone.png` | 영어 캡처 완료 |

## `en/billing/billing.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-subscription.png` | 영어 캡처 완료 |
| `en-subscription-service.png` | 영어 캡처 완료 |
| `en-subscription-annual.png` | 영어 캡처 완료 |
| `en-subscription-payment-method.png` | 영어 캡처 완료 |
| `en-subscription-change-payment-date.png` | 영어 캡처 완료 |
| `en-subscription-detail.png` | 영어 캡처 완료 |
| `en-subscription-manage.png` | 영어 캡처 완료 |
| `en-subscription-history.png` | 영어 캡처 완료 |
| `en-subscription-history-detail.png` | 영어 캡처 완료 |

## `en/bookings/bookings.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-bookings-default.png` | 영어 캡처 완료 |
| `en-booking-list-view-2.png` | 영어 캡처 완료 |
| `en-booking-card-view-2.png` | 영어 캡처 완료 |
| `en-booking-csv-download-2.png` | 영어 캡처 완료 |

## `en/channels/agoda.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image(55).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(56).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(57).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(58).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/airbnb.mdx`

| 이미지 | 상태 |
| --- | --- |
| `airbnb-host-mode-4.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `chrome-extension.png` | 한국어 유지 — 크롬 브라우저 확장 프로그램 메뉴다. VCMS 화면이 아니다 |
| `airbnb-sign-in-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/camfit.mdx`

| 이미지 | 상태 |
| --- | --- |
| `camfit-1-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `camfit-2.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/camperest.mdx`

| 이미지 | 상태 |
| --- | --- |
| `camperest_01.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/channel-setting.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-channel-connected-list.png` | 영어 캡처 완료 |
| `en-channels-available.png` | 영어 캡처 완료 |
| `en-channel-connect-form.png` | 영어 캡처 완료 |
| `channel-property-info-none-connected.png` | 재현 불가 — 채널 연결 플로우 중간 단계라 실제로 연결을 눌러야 나온다. 운영 접근은 조회만 허용이라 못 찍는다(Dean, 2026-09-13) |
| `channel-property-connect-1.png` | 재현 불가 — 채널 연결 플로우 중간 단계라 실제로 연결을 눌러야 나온다. 운영 접근은 조회만 허용이라 못 찍는다(Dean, 2026-09-13) |
| `en-channel-property-info.png` | 영어 캡처 완료 |
| `en-channel-commission-modal.png` | 영어 캡처 완료 |
| `en-channel-final-rate-modal.png` | 영어 캡처 완료 |
| `en-channel-packages.png` | 영어 캡처 완료 |
| `en-channel-package-edit.png` | 영어 캡처 완료 |
| `en-channel-package-visibility.png` | 영어 캡처 완료 |

## `en/channels/expedia.mdx`

| 이미지 | 상태 |
| --- | --- |
| `expedia-partner-central-4.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `chrome-extension-reconnect-7.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `expedia-reconnect-notification-6.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `expedia-partner-center-obp.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `expedia-partner-center.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/liveanywhere.mdx`

| 이미지 | 상태 |
| --- | --- |
| `liveanywhere-min-nights-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/naver.mdx`

| 이미지 | 상태 |
| --- | --- |
| `naver-2FA-deactivate-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `naver-2FA-deactivate-3.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `naver-2FA-deactivate-2-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `naver-2FA-deactivate-web.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `naver-connect.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-5.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/tripbtoz.mdx`

| 이미지 | 상태 |
| --- | --- |
| `tripbtoz-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `tripbtoz-2.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/tripdotcom.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image(59).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(61).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(63).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(64).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/channels/yanolja.mdx`

| 이미지 | 상태 |
| --- | --- |
| `notification.png` | 재현 불가 — 채널 상품 알림이 있어야 한다. VENDIT HOTEL 은 알림 0건이고, dev 는 여는 순간 읽음 처리될 수 있어서 안 열었다(2026-09-12) |

## `en/distributions/channel-sync.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-distribution-syncing.png` | 영어 캡처 완료 |
| `Distribution-Booking-Sync-Only.png` | Dean 확인 필요 — 영어 배너가 {date} 를 그대로 찍는다. dev 번들 en.json 이 낡았고 vcms-i18n main 은 {launchedAt} 로 고쳐져 있다. en-distribution-booking-sync-only.png 준비됨, 배포 후 교체 |
| `260727-channel-example.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/distributions/understanding.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-legend-sell-status-2.png` | 영어 캡처 완료 |
| `en-legend-sold-inventory.png` | 영어 캡처 완료 |
| `en-legend-rate-adjustment.png` | 영어 캡처 완료 |
| `en-legend-rate-status-2.png` | 영어 캡처 완료 |
| `en-legend-availability-2.png` | 영어 캡처 완료 |
| `en-legend-change-status-2.png` | 영어 캡처 완료 |

## `en/distributions/view-setting.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-distribution-filter.png` | 영어 캡처 완료 |
| `en-view-settings.png` | 영어 캡처 완료 |

## `en/faq/booking/cancelled-booking-no-history.mdx`

| 이미지 | 상태 |
| --- | --- |
| `faq-no-history.png` | 재현 불가 — 수집 시점에 이미 취소된 예약이 있어야 한다. VENDIT HOTEL 기본 조회 기간 Canceled 0건(2026-09-11) |

## `en/faq/booking/channel-cancel-booking-processing.mdx`

| 이미지 | 상태 |
| --- | --- |
| `cancel-booking-noti.png` | 재현 불가 — 카카오 알림톡 원문이다. 한국어로만 발송된다 |

## `en/faq/channels/airbnb-promotion-reset.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image-34.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `KakaoTalk_Photo_2026-04-21-18-18-23-001-3.jpeg` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-(1).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `KakaoTalk_Photo_2026-04-21-18-18-23-002-2.jpeg` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/faq/channels/available-channels.mdx`

| 이미지 | 상태 |
| --- | --- |
| `channels/yanolja.svg` | 채널 로고 (언어 무관) |
| `channels/yeogi.svg` | 채널 로고 (언어 무관) |
| `channels/coolstay.svg` | 채널 로고 (언어 무관) |
| `channels/coolstay-pension.svg` | 채널 로고 (언어 무관) |
| `channels/onda-pension.svg` | 채널 로고 (언어 무관) |
| `channels/tripbtoz.svg` | 채널 로고 (언어 무관) |
| `channels/liveanywhere.svg` | 채널 로고 (언어 무관) |
| `channels/banlife.svg` | 채널 로고 (언어 무관) |
| `channels/naver.svg` | 채널 로고 (언어 무관) |
| `channels/yeogi-partner.svg` | 채널 로고 (언어 무관) |
| `channels/coolstay-hotel.svg` | 채널 로고 (언어 무관) |
| `channels/ddnayo.svg` | 채널 로고 (언어 무관) |
| `channels/onda-hotel.svg` | 채널 로고 (언어 무관) |
| `channels/camfit.svg` | 채널 로고 (언어 무관) |
| `channels/camperest.svg` | 채널 로고 (언어 무관) |
| `channels/agoda.svg` | 채널 로고 (언어 무관) |
| `channels/airbnb.svg` | 채널 로고 (언어 무관) |
| `channels/trip.svg` | 채널 로고 (언어 무관) |
| `channels/expedia.svg` | 채널 로고 (언어 무관) |

## `en/faq/integrations/vcloud.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-integrations.png` | 영어 캡처 완료 |
| `integration-vcloud-2.png` | 재현 불가 — 찍을 필요 없음. VCLOUD 는 점주가 설정 못 하게 막아둔 게 의도다(Dean, 2026-09-12). 한국어 가이드도 같은 관리자 매뉴얼 절을 갖고 있다 |
| `en-vcloud-failed.png` | 영어 캡처 완료 |
| `integration-vcloud-4.png` | 재현 불가 — 찍을 필요 없음. VCLOUD 는 점주가 설정 못 하게 막아둔 게 의도다(Dean, 2026-09-12) |

## `en/faq/inventory-rate/airbnb-promotion-reset.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image-34.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `KakaoTalk_Photo_2026-04-21-18-18-23-001-3.jpeg` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-(1).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `KakaoTalk_Photo_2026-04-21-18-18-23-002-2.jpeg` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/faq/inventory-rate/bulk-edit-period-inventory.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-bulk-update-button.png` | 영어 캡처 완료 |
| `en-bulk-update-modal.png` | 영어 캡처 완료 |
| `en-bulk-update-added.png` | 영어 캡처 완료 |
| `bulk-edit-4.png` | 재현 불가 — 임시 변경 바는 일괄변경이 아니라 셀 직접 수정 플로우 것이다 |
| `bulk-edit-5.png` | 재현 불가 — 임시 변경 바는 일괄변경이 아니라 셀 직접 수정 플로우 것이다 |

## `en/faq/inventory-rate/bulk-period-rate-adjustment.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-period-rate-weighting.png` | 영어 캡처 완료 |
| `en-pkg-period-weighting.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/change-room-count.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-roomtype-pick.png` | 영어 캡처 완료 |
| `en-roomtype-add-room.png` | 영어 캡처 완료 |
| `en-roomtype-room-status.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/channel-final-rate.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-channel-property-info.png` | 영어 캡처 완료 |
| `en-channel-final-rate-modal.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/channel-inventory-rate-setting.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-channel-packages.png` | 영어 캡처 완료 |
| `en-channel-package-visibility.png` | 영어 캡처 완료 |
| `en-channel-package-select.png` | 영어 캡처 완료 |
| `en-channel-package-edit.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/channels-commission-setting.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-channel-property-info.png` | 영어 캡처 완료 |
| `en-channel-commission-modal.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/limit-inventory-by-channel.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-channel-packages.png` | 영어 캡처 완료 |
| `en-channel-package-select.png` | 영어 캡처 완료 |
| `en-channel-package-edit.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/timetables.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-timetable-create.png` | 영어 캡처 완료 |
| `en-timetable-weekday.png` | 영어 캡처 완료 |
| `en-timetable-packages.png` | 영어 캡처 완료 |

## `en/faq/inventory-rate/yanolja-hotdeal-goldclass.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image(65).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image(66).png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/faq/issue/airbnb-disconnected.mdx`

| 이미지 | 상태 |
| --- | --- |
| `airbnb-sign-in-2.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `expedia-partner-center-1.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `chrome-extension-1.png` | 한국어 유지 — 크롬 브라우저 확장 프로그램 메뉴다. VCMS 화면이 아니다 |
| `image-35.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-36.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-37.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-39.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-38.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/faq/issue/channel-auto-disconnected.mdx`

| 이미지 | 상태 |
| --- | --- |
| `channel-auto-disconnect-noti.png` | 재현 불가 — 카카오 알림톡 원문이다. 한국어로만 발송된다 |

## `en/faq/issue/vcloud-booking-not-synced.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-booking-confirm-button.png` | 영어 캡처 완료 |

## `en/faq/issue/yanolja-booking-delay.mdx`

| 이미지 | 상태 |
| --- | --- |
| `blocked.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `yanolja-system-error-modal-1.png` | 재현 불가 — 야놀자 장애 때만 뜨는 모달이라 재현 불가 |
| `en-accommodation-info.png` | 영어 캡처 완료 |
| `image-10.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `image-9.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/inventory-rate/inclusion.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-inclusion-list.png` | 영어 캡처 완료 |
| `en-inclusion-create.png` | 영어 캡처 완료 |
| `en-inclusion-delete.png` | 영어 캡처 완료 |
| `en-inclusion-delete-confirm.png` | 영어 캡처 완료 |

## `en/inventory-rate/package-day-use.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-dayuse-list.png` | 영어 캡처 완료 |
| `en-dayuse-create.png` | 영어 캡처 완료 |
| `en-dayuse-delete-icon.png` | 영어 캡처 완료 |
| `en-dayuse-delete-confirm.png` | 영어 캡처 완료 |

## `en/inventory-rate/package-stay.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-pkg-list.png` | 영어 캡처 완료 |
| `en-pkg-list-rate.png` | 영어 캡처 완료 |
| `en-create-pkg.png` | 영어 캡처 완료 |
| `booking_scaleFactor.png` | 재현 불가 — 배수가 걸린 예약 두 건을 나란히 놓은 합성이다 |
| `en-create-pkg-rate.png` | 영어 캡처 완료 |
| `en-create-pkg-rate-select.png` | 영어 캡처 완료 |
| `create-pkg-inclusions.png` | 재현 불가 — 현재 빌드 상품 모달에 서비스 선택이 없다. 서비스는 요금에 붙는다(en-rate-inclusions). 문서가 낡았다 |
| `en-create-pkg-rate-scale.png` | 영어 캡처 완료 |
| `en-create-pkg-rate-range.png` | 영어 캡처 완료 |
| `en-create-pkg-obp.png` | 영어 캡처 완료 |
| `en-pkg-rate-periods.png` | 영어 캡처 완료 |
| `en-pkg-add-unlinked-periods.png` | 영어 캡처 완료 |
| `en-pkg-add-period.png` | 영어 캡처 완료 |
| `en-pkg-bulk-rate-plan-modal.png` | 영어 캡처 완료 |
| `en-pkg-bulk-rate-plan-button.png` | 영어 캡처 완료 |
| `en-pkg-import-rate-plan.png` | 영어 캡처 완료 |
| `pkg_delete.png` | Dean 확인 필요 — 현재 빌드는 휴지통이 비활성으로 안 바뀐다. 눌러서 뜨는 대화상자를 찍어야 하는데 운영 삭제 버튼이라 자동 모드가 막았다 |

## `en/inventory-rate/period.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-period-list.png` | 영어 캡처 완료 |
| `en-period-create.png` | 영어 캡처 완료 |
| `en-period-rate-adjust.png` | 영어 캡처 완료 |
| `en-period-delete-icon.png` | 영어 캡처 완료 |
| `en-period-delete-confirm.png` | 영어 캡처 완료 |

## `en/inventory-rate/rate.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-rate-list.png` | 영어 캡처 완료 |
| `en-rate-list-expanded.png` | 영어 캡처 완료 |
| `en-rate-create.png` | 영어 캡처 완료 |
| `en-rate-inclusions.png` | 영어 캡처 완료 |
| `en-rate-delete-icon.png` | 영어 캡처 완료 |
| `en-rate-delete-confirm.png` | 영어 캡처 완료 |

## `en/inventory-rate/roomType.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-roomtype-list.png` | 영어 캡처 완료 |
| `en-roomtype-list-unfold.png` | 영어 캡처 완료 |
| `en-create-roomtype.png` | 영어 캡처 완료 |
| `edit-roomtype-confirm.png` | 재현 불가 — CMS 관리 객실타입 + 범위를 벗어난 연결 상품이 필요하다 |

## `en/inventory-rate/timetable.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-timetable-list.png` | 영어 캡처 완료 |
| `en-timetable-create.png` | 영어 캡처 완료 |
| `en-timetable-weekday.png` | 영어 캡처 완료 |
| `en-timetable-packages.png` | 영어 캡처 완료 |
| `en-timetable-delete.png` | 영어 캡처 완료 |
| `en-timetable-delete-confirm.png` | 영어 캡처 완료 |

## `en/release-notes/2025-12-23.mdx`

| 이미지 | 상태 |
| --- | --- |
| `change-scope-desktop.png` | 한국어 보존 — 현재 빌드는 상위 토글을 켜면 하위로 바로 내려간다. '하위 변경 항목' 팝오버가 없다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `change-scope-mobile.png` | 한국어 보존 — 같은 기능. 데스크톱에서 확인 단계가 사라졌다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `fixed-rate-protection.png` | 영어 재캡처 필요 |
| `package-fixed-rate-setting.png` | 한국어 보존 — 상품 생성 단계가 3개(기본 정보/요금 정보/기간별 요금 설정)에서 2개(Basic information/Rate plan)로 줄었다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `rate-period-settings.png` | 영어 재캡처 필요 |

## `en/release-notes/2026-01-14.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image-17.png` | 한국어 보존 — 요금 조절 모달이 Rate adjustment 로 전면 개편됐다. 평균 요금·초기화/저장 대신 OCC 배지·Amount/Percent·객실 타입 그룹. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `image-12.png` | 한국어 보존 — 판매 중지 확인 모달(개별 설정 유지)이 없다. 임시 변경 바로 바로 간다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `image-11.png` | 한국어 보존 — 판매 재개 확인 모달도 같이 없어졌다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `image-14.png` | 재현 불가 — VCMS 아고다 연동 로그인 화면인데 아고다 채널 연결 버튼을 눌러야 나온다. 운영에서 채널 연결은 안 누른다 |
| `image-16.png` | 한국어 유지 — 크롬 브라우저 확장 프로그램 메뉴다. VCMS 화면이 아니다 |

## `en/release-notes/2026-01-29.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image-25.png` | 한국어 보존 — 그리드 행 구성이 바뀌었다. 전체/판매 완료 → Availability/Sold/Adjustment/Check-in/out. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `image-26.png` | 한국어 보존 — 채널 상품 정보 수정 모달이 없다. 수수료·전송가는 채널 > 숙소 정보로 옮겼다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |

## `en/release-notes/2026-02-27.mdx`

| 이미지 | 상태 |
| --- | --- |
| `image-27.png` | 영어 재캡처 필요 |
| `image-29.png` | 영어 재캡처 필요 |
| `image-30.png` | 영어 재캡처 필요 |
| `image-28.png` | 재현 불가 — 잘린 업장 이름을 툴팁으로 펼친 사진이다. 그 이름이 바로 가려야 하는 정보라 영어로 다시 찍어도 쓸 수 없다 |

## `en/release-notes/2026-03-06.mdx`

| 이미지 | 상태 |
| --- | --- |
| `yanolja-channel-package-1.png` | 한국어 보존 — 채널 상품 정보 수정 모달이 없다(그리드 행·연필·링크·채널 상품 탭 다섯 경로 확인). 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `agoda-derived-1.png` | 한국어 보존 — '이미 예약된 순 객실 박수' '예약 가능한 잔여 객실 박수' 행이 현재 그리드에 없다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `agoda-channel-package-1.png` | 한국어 보존 — 같은 모달. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `yeogi-channel-pacakge-1.png` | 한국어 보존 — 같은 모달. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `no-property-connected-label.png` | 재현 불가 — '연결한 숙소 없음' 배지 상태를 만들 수 없다. VENDIT HOTEL 은 연결 0, 대구 아르코는 6채널 전부 숙소가 붙어 있다 |

## `en/release-notes/2026-03-16.mdx`

| 이미지 | 상태 |
| --- | --- |
| `CMS-Channel-Talk.png` | 영어 재캡처 필요 |

## `en/release-notes/2026-04-24.mdx`

| 이미지 | 상태 |
| --- | --- |
| `airbnb-7.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `expedia-7.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `Banner-Agoda-Referral-1.png` | 한국어 보존 — 아고다 입점 배너가 현재 빌드에 없다(채널·판매관리·숙박업소 목록 확인). 캠페인 배너라 내려간 것으로 본다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `260423-Import-Channel-Pkg-1.png` | 영어 재캡처 필요 |
| `260423-Channel-2.png` | 영어 재캡처 필요 |
| `Update-Note-Subscription-1.png` | 재현 불가 — 유료 구독 상태 화면이다. 쓸 수 있는 업장 둘 다 무료 체험이고 구독 버튼은 안 누른다 |

## `en/release-notes/2026-04-28.mdx`

| 이미지 | 상태 |
| --- | --- |
| `camfit.png` | 채널 로고 (언어 무관) |

## `en/release-notes/2026-04-29.mdx`

| 이미지 | 상태 |
| --- | --- |
| `Yanolja-Hardblock-1.png` | 한국어 보존 — 예약 표가 전면 개편됐다. 매출액 합계·제휴점 예약번호·상세보기·페이지네이션이 없고 카드/목록 전환이 생겼다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |

## `en/release-notes/2026-05-15.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-distribution-channel-disconnected.png` | 영어 캡처 완료 |
| `260515-High-Price.png` | 재현 불가 — 1억 넘는 요금이 설정된 상품이 있어야 한다. 임시 변경으로 만들면 그리드가 +130,00... 으로 잘린다. 억 축약은 셀 폭 때문에 있는 것이다 |
| `en-260515-roomtype-max.png` | 영어 캡처 완료 |
| `260515-Mobile-Channel.png` | 재현 불가 — 폰 프레임에 스크린샷 3장을 얹은 합성이다. 화면 하나가 아니라 디자인 작업이다 |
| `260515-Mobile-Channel-Mapping.png` | 재현 불가 — 같은 합성. 폰 프레임 3장 |
| `en-260515-subscription.png` | 영어 캡처 완료 |
| `260515-Billing-Cancel.png` | 재현 불가 — 구독 취소 버튼을 눌러야 나온다. 구독/결제 버튼은 안 누른다 |
| `en-260515-language.png` | 영어 캡처 완료 |

## `en/release-notes/2026-06-09.mdx`

| 이미지 | 상태 |
| --- | --- |
| `liveanywhere-2.png` | 채널 로고 (언어 무관) |
| `liveanywhere-min-nights-2.png` | 재현 불가 — 최소 박수 개념을 설명하려고 그린 월 달력 도해다. VCMS 화면 캡처가 아니라 디자인 작업이다 |

## `en/release-notes/2026-06-17.mdx`

| 이미지 | 상태 |
| --- | --- |
| `260617-Booking-Multiplier-Setting-2.png` | 한국어 보존 — 상품 생성 모달과 예약 상세가 둘 다 바뀐 합성이다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `260617-Booking-OTA-Example-2.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |
| `260615-Rate-Setting.png` | 영어 재캡처 필요 |
| `260615-Roomtype-Setting.png` | 영어 재캡처 필요 |
| `260609-Booking.png` | 재현 불가 — 데스크톱 예약 상세와 폰 프레임을 나란히 얹은 합성이다. 화면 하나가 아니다 |

## `en/release-notes/2026-06-29.mdx`

| 이미지 | 상태 |
| --- | --- |
| `Camperest-1.png` | 채널 로고 (언어 무관) |
| `banlife-stay.png` | 채널 로고 (언어 무관) |

## `en/release-notes/2026-07-02.mdx`

| 이미지 | 상태 |
| --- | --- |
| `260630-Inventory-Settings.png` | 영어 재캡처 필요 |
| `260630-Booking-Status-1.png` | 영어 재캡처 필요 |

## `en/release-notes/2026-07-07.mdx`

| 이미지 | 상태 |
| --- | --- |
| `260707-Inventory-Bulk-Update-1.png` | 재현 불가 — 셀 드래그 복사는 CDP 로 재현이 안 된다(2026-09-14 재확인) |
| `260707-Inventory-Bulk-Update-2.png` | 재현 불가 — 같은 이유. 주말만 드래그도 CDP 로 안 된다 |

## `en/release-notes/2026-07-24.mdx`

| 이미지 | 상태 |
| --- | --- |
| `260724-distribution-free-trial-subscribe-2.png` | 재현 불가 — 서비스 구독 모달이다. 구독 신청 버튼을 눌러야 나온다 |
| `260724-distribution-free-trial-subscribe-1-1.png` | 한국어 보존 — 판매관리 우상단 '무료 체험' 버튼이 제목 옆 배지로 바뀌었다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `260724-subscription-free-plan-subscribe-1-1.png` | 한국어 보존 — 구독 페이지 섹션이 기본 제공/이용 가능한 서비스 두 개에서 하나로 합쳐졌다. 지금 화면으로 다시 찍으면 그 릴리즈 기록이 아니게 된다(Dean, 2026-09-14) |
| `260724-subscription-free-plan-subscribe-2-1.png` | 재현 불가 — 구독 구성 선택 모달. 같은 이유 |
| `260724-subscription-paid-plan-manage-1-1.png` | 재현 불가 — 유료 구독 상태가 필요하다 |
| `260724-subscription-billing-date-change-1.png` | 재현 불가 — 결제일 변경 모달. 결제 버튼은 안 누른다 |
| `260724-subscription-paid-plan-manage-3-1.png` | 재현 불가 — 구독 변경 모달. 같은 이유 |
| `260724-subscription-payment-history-1-1.png` | 재현 불가 — 결제 내역이 있어야 한다. 두 업장 다 0건(2026-09-14 확인) |
| `260724-subscription-payment-history-2.png` | 재현 불가 — 결제 상세 모달. 같은 이유 |

## `en/release-notes/2026-07-27.mdx`

| 이미지 | 상태 |
| --- | --- |
| `260727-channel-example.png` | 한국어 유지 — 채널사가 만든 화면이라 범위 밖이다. 본문 영어 번역만 하고 사진은 한국어 그대로 둔다(Dean, 2026-09-13) |

## `en/release-notes/2026-08-07.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-260807-search-accommodation.png` | 영어 캡처 완료 |
| `en-260807-bookings-card-view.png` | 영어 캡처 완료 |
| `en-260807-edit-pkg-modal.png` | 영어 캡처 완료 |
| `en-260807-edit-pkg-rate.png` | 영어 캡처 완료 |

## `en/release-notes/2026-08-20.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-260820-rate-adjustment.png` | 영어 캡처 완료 |
| `260820-rate-drag-2.png` | 재현 불가 — 셀 드래그 복사는 CDP 로 재현이 안 된다. mousePressed/mouseMoved/mouseReleased 를 쏴도 복사 메뉴가 안 뜬다(2026-09-14 재확인) |
| `en-260820-rate-scale-factor.png` | 영어 캡처 완료 |
| `260820-timetable-drag.png` | 재현 불가 — 같은 이유. 입퇴실 시간 드래그도 CDP 로 안 된다 |
| `en-260820-timetable-shown.png` | 영어 캡처 완료 |
| `260820-scale-factor-disabled.png` | 재현 불가 — 객실이 1개인 객실 타입이 있어야 예약 배수가 비활성으로 뜬다. VENDIT HOTEL 최소 객실 타입이 2개다 |

## `en/release-notes/2026-09-03.mdx`

| 이미지 | 상태 |
| --- | --- |
| `en-260903-bulk-apply-modal.png` | 영어 캡처 완료 |
| `en-260903-bulk-apply-button.png` | 영어 캡처 완료 |
| `en-260903-import-rate-plan.png` | 영어 캡처 완료 |
| `en-260903-add-multi-period.png` | 영어 캡처 완료 |
| `en-260903-add-multi-period-modal.png` | 영어 캡처 완료 |
| `en-260903-single-add-period.png` | 영어 캡처 완료 |
| `en-260903-bulk-align-button.png` | 영어 캡처 완료 |
| `en-260903-single-align-order.png` | 영어 캡처 완료 |
| `en-260903-expand-collapse.png` | 영어 캡처 완료 |
| `en-260903-roomtype-filter-multi.png` | 영어 캡처 완료 |
| `en-260903-temporary-change-bar.png` | 영어 캡처 완료 |

